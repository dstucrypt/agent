...
function connect({host, port, key}) {
  console.log('connect to', host, port, key)
  const socket = new net.Socket();
  const frame = new Frame(key);
  return new Promise((resolve, reject)=> {
    socket.connect(port, host, function connected() {
      const back = {
        port,
        key,
        host,
...
async function get_back(bid, retry=1) {
  ...
  if(ret && ret.frame.conn.destroyed) {
    try {
      ret = await connect({port: ret.port, key: ret.key, host: ret.key});
    } catch(e) {
      ret = null;
    }
  }
...
var start = function (opts) {
    ...
    var port = lport;
    while(port <= hport) {
      backs.list.push({port, key, host: opts.host});
      port++;
    }
