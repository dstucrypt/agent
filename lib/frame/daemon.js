...
var start = function (opts) {
  ...
  var server = net.createServer(function (conn) {
    ...
    frame.on("msg", conn, async function (contents, type) {
      ...
      if (type === "octstr") {
        if (this._data) { 
          //if this._data already exists then client is sending another file with a content being verified
        ...
        try {
          box.load({ password: decodeStr(contents.password), keyBuffers: [this._data] });
          delete this._data;
          delete this._data2
        ...
        try {
          const data = await box.pipe(this._data, contents.pipe, contents.opts);
          delete this._data;
          delete this._data2
        ...
        try {
          const info = await box.unwrap(this._data, this._data2 || undefined, contents.opts);
          delete this._data;
          delete this._data2
