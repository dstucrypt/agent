...
var start = function (opts) {
  ...
  var server = net.createServer(function (conn) {
    ...
    frame.on("msg", conn, function (contents, type) {
      ...
      if (type === "octstr") {
        if (this._data) { //if _data has bytes then client is sending another file with content being verified
          this._data2 = contents;
        } else {
          this._data = contents;
        }
        ...
        delete this._data;
        if (this._data2) {
          //to verify the signature of a standalone file client sends us two files: p7s and a content file
          delete this._data2
        }
        ...
        var contentBytes = undefined
        if (this._data2) {
          //to verify the signature of a standalone file client sends us two files: p7s and a content file
          contentBytes = this._data2
        }
        box.unwrap(this._data, contentBytes, contents.opts).then(
          (info) => {
            delete this._data;
            if (this._data2) {
              //to verify the signature of a standalone file client sends us two files: p7s and a content file
        ...
        box.findCertsCmp(contents.urls).then(
          (number) => {
